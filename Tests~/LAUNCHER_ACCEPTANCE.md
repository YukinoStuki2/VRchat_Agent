...
8. Connect, then close launcher window, trigger ordinary script compilation, and quit Unity separately. Each must revoke managed permissions and request/complete owned cleanup. Reload must not silently reconnect. If cleanup is unconfirmed, a new start stays blocked; record actual diagnostic.
